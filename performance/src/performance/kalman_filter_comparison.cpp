...
bool kalman_filter_comparison(
    ...
    // Collect truth data for comparison
    ...
    traccc::edm::track_container<algebra_t>::host track_container_fw{host_mr};
    traccc::edm::track_container<algebra_t>::host track_container_bw{host_mr};
    ...
    for (std::size_t i_event = 0u; i_event < n_events; ++i_event) {
        ...
        for (const auto& [ptc_id, ptc] : evt_data.m_particle_map) {
            ...
            for (const auto& [i, meas] :
                ...
                const auto state_idx{static_cast<unsigned int>(
                    track_container_fw.states.size())};

                track.constituent_links().push_back(
                    {edm::track_constituent_link::track_state, state_idx});

                track_container_fw.states.push_back(
                    edm::make_track_state<algebra_t>(device_measurements,
                                                     meas_idx));
            }

            // Record the truth track candidate.
            track_container_fw.tracks.push_back(track);
            track_container_bw.tracks.push_back(track);

            TRACCC_DEBUG("-> Truth tracks "
                         << track_container_fw.tracks.size());
        ...
        if (tracks.size() - n_tracks == 0u) {
            ...
            TRACCC_VERBOSE("Event "
                           << i_event << ": Found " << tracks.size() - n_tracks
                           << " reconstructible truth track(s) in event");
        }
    }

    // Make the final measurement info available to the input containers
    track_container_fw.measurements = vecmem::get_data(measurements);
    track_container_bw.measurements = vecmem::get_data(measurements);
    ...
    if (track_container_fw.tracks.size() == 0u) {
        TRACCC_ERROR("Kalman Filter truth data empty");
        return false;
    }
    if (track_container_fw.tracks.size() != tracks.size()) {
        TRACCC_ERROR(
            "Initial track parameters and KF (forward) tracks do not match");
        return false;
    }

    // Copy the forward track states in reverse order for the backward KF
    track_container_bw.states = track_container_fw.states;
    /*for (std::size_t i = 0u; i < track_container_fw.tracks.size(); ++i) {
        const auto trk_fw = track_container_fw.tracks.at(i);
        edm::track_collection<algebra_t>::host::object_type track;

        if (trk_fw.constituent_links().size() == 0u) {
            continue;
        }

        for (int j = static_cast<int>(trk_fw.constituent_links().size()) - 1; j
    >= 0; --j) { track.constituent_links().push_back(
                trk_fw.constituent_links().at(static_cast<unsigned int>(j)));
        }

        track_container_bw.tracks.push_back(track);
    }*/

    if (track_container_bw.tracks.size() != tracks.size()) {
        TRACCC_ERROR(
            "Initial track parameters and KF (backward) tracks do not match");
        return false;
    }
    ...
    test_cfg.ptc_hypothesis(ptc_type);
...

    {
        ...
        typename edm::track_container<algebra_t>::data tracks_data{
            track_container_fw};
        ...
        vecmem::vector<typename actor_chain_t::state_ref_tuple>
            state_ref_tuple{};
        state_tuple.reserve(track_container_fw.tracks.size());
        state_ref_tuple.reserve(track_container_fw.tracks.size());
        ...
        const auto [trk_stats_fw, n_surfaces_fw, n_miss_nav_fw, n_miss_truth_fw,
                    step_traces_fw, mat_traces_fw, mat_records_fw] =
            detray::navigation_validator::compare_to_navigation<
                stepper_t, parameter_updater>(
                test_cfg, host_mr, det, names, ctx, field_view, prop_cfg,
                truth_traces_fw_KF, tracks, state_ref_tuple);
        ...
        // Create the input container(s) the way the KF needs them
        typename edm::track_container<algebra_t>::data tracks_data_bw{
            track_container_bw};
        typename edm::track_container<algebra_t>::view tracks_view_bw{
            tracks_data_bw};
        typename edm::track_container<algebra_t>::device
            device_track_container_bw{tracks_view_bw};
        ...
        for (std::size_t i = 0u; i < tracks.size(); ++i) {
            ...
            fit_actor_bw::state fit_actor_state(
                device_track_container_bw.tracks.at(
                    static_cast<unsigned int>(i)),
                device_track_container_bw.states, device_measurements,
                device_sf_sequence, calib_cfg);
            ...
            state_tuple_bw.push_back(
                detray::make_tuple(stopper_state, interactor_state,
                                   fit_actor_state, updater_state_bw));
        ...
        const auto [trk_stats_bw, n_surfaces_bw, n_miss_nav_bw, n_miss_truth_bw,
            ...
            detray::navigation_validator::compare_to_navigation<
                ...
                test_cfg, host_mr, det, names, ctx, field_view, prop_cfg,
                truth_traces_bw_KF, tracks, state_ref_tuple_bw);
