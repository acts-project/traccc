...
constexpr std::size_t n_tracks{100u};
...
class KF_intergration_test_toy_detector
    : public ToyDetectorFixture,
      public ::testing::WithParamInterface<
          std::tuple<float, float, bool, bool, bool>> {};
...
TEST_P(KF_intergration_test_toy_detector, toy_detector) {
    ...
    prop_cfg.stepping.path_limit = 2. * traccc::unit<scalar>::m;

    const bool success = kalman_filter_comparison(
        &host_det, names, field, prop_cfg, outdir, n_events, logger->clone(),
        std::get<3>(GetParam()), std::get<4>(GetParam()), false, ptc_type, {},
        min_p, max_r);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(std::make_tuple(0.5f * traccc::unit<scalar>::GeV,
                                      1e-5f * traccc::unit<float>::mm, true,
                                      false, true)));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(std::make_tuple(100.f * traccc::unit<scalar>::GeV,
                                      1.5f * traccc::unit<float>::mm, true,
                                      true, true)));
INSTANTIATE_TEST_SUITE_P(
