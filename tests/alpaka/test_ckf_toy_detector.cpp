...
TEST_P(CkfToyDetectorTests, Run) {
    ...
    traccc::alpaka::combinatorial_kalman_filter_algorithm::config_type cfg;
    ...
    for (std::size_t i_evt = 0; i_evt < n_events; i_evt++) {
        ...
        // Run device finding
        auto track_candidates_alpaka_buffer = device_finding(
        ...
        ASSERT_LE(static_cast<double>(std::llabs(
                      ...
                      static_cast<long>(track_candidates_alpaka.size()))) /
        ...
        for (unsigned int i = 0u; i < track_candidates.tracks.size(); i++) {
            ...
            for (unsigned int j = 0u; j < track_candidates_alpaka.size(); j++) {
                if (iso(track_candidates_alpaka.at(j))) {
                    n_matches++;
                    break;
                }
            }
...
INSTANTIATE_TEST_SUITE_P(
    AlpakaCkfToyDetectorValidation, CkfToyDetectorTests,
    ::testing::Values(
