...
/// Copy vector to device, set element[i] = sin(i) on device, then compare with same calculation on host
GTEST_TEST(AlpakaBasic, VectorOp) {
    ...
    alpaka::exec<Acc>(queue, workDiv, VectorOpKernel{}, alpaka::getPtrNative(bufAcc), n);
