...
class ConnectedComponentAnalysisTests
    ...
    inline void test_connected_component_analysis(ParamType p) {
        ...
        if (cluster_data.has_value()) {
            ...
            std::vector<unsigned int> disjoint_set;
            disjoint_set.resize(cells.size());

            for (unsigned int i = 0; i < cluster_data->size(); ++i) {
                for (unsigned int j = 0;
                     j < cluster_data->cell_indices().at(i).size(); ++j) {
                    disjoint_set.at(cluster_data->cell_indices().at(i).at(j)) =
                        i;
                }
            }

            auto cell_reader = traccc::io::csv::make_cell_reader(file_hits);
            traccc::io::csv::cell iocell;
            std::size_t cell_idx = 0;
            std::map<unsigned int, uint64_t> disjoint_to_measidx_map;
            std::map<uint64_t, unsigned int> measidx_to_disjoint_map;
            while (cell_reader.read(iocell)) {
                unsigned int disjoint_set_value = disjoint_set.at(cell_idx);
                cell_idx++;

                auto it1 = disjoint_to_measidx_map.find(disjoint_set_value);
                auto it2 = measidx_to_disjoint_map.find(iocell.measurement_id);

                if (it1 != disjoint_to_measidx_map.end()) {
                    EXPECT_EQ(it1->second, iocell.measurement_id);
                } else {
                    disjoint_to_measidx_map.emplace(disjoint_set_value,
                                                    iocell.measurement_id);
                }

                if (it2 != measidx_to_disjoint_map.end()) {
                    EXPECT_EQ(it2->second, disjoint_set_value);
                } else {
                    measidx_to_disjoint_map.emplace(iocell.measurement_id,
                                                    disjoint_set_value);
                }
            }
        }
