...
/// Templated implementation of the SYCL track fitting algorithm with fitted
/// tracks.
///
...
kalman_fitting(
    ...
    ::sycl::queue& queue, bool forward_on_first_iteration = false) {
...
/// Templated implementation of the SYCL track fitting algorithm with unfitted
/// tracks.
///
/// @tparam kernel_t   Structure to generate unique kernel names with
/// @tparam detector_t The (device) detector type to use
    ...
    queue
        .submit([&](::sycl::handler& h) {
            h.parallel_for<kernels::fit_prelude<kernel_t>>(
                range, [track_candidates_view,
                        track_states_view](::sycl::nd_item<1> item) {
                    device::fit_prelude<typename detector_t::algebra_type>(
                        details::global_index(item), track_candidates_view,
                        track_states_view);
                });
        })
        .wait_and_throw();

    return kalman_fitting<kernel_t, detector_t, bfield_t>(
        det_view, field_view,
        typename edm::track_fit_container<
