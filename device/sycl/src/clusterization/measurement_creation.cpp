...
host_measurement_container measurement_creation::operator()(
    ...
    // Vector with for counts of clusters per each module
    vecmem::vector<unsigned int> cluster_sizes(num_modules, 1, &m_mr.get());

    // Atomic count of all clusters (needed inside component connection kernel but also here)
    vecmem::vector<unsigned int> total_clusters(1, 0, &m_mr.get());

    traccc::sycl::component_connection(
        clusters_buffer, get_data(cells_per_event, &m_mr.get()), cells_per_event,
        vecmem::get_data(cluster_sizes), vecmem::get_data(total_clusters), m_mr.get(), m_queue);
