...
void seed_selecting(
    ...
    triplet triplets_per_spM[sh_mem];
    Kokkos::parallel_for("seed_selecting", team_policy(num_blocks, num_threads),
        KOKKOS_LAMBDA (const member_type &team_member) {
            ...
            device_seed_collection seed_device(seed_buffer);

            // Get the bin and item index
            unsigned int bin_idx(0), item_idx(0);
            kokkos_helper::find_idx_on_container(doublet_counter_device,
