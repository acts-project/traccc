...
__global__ void is_ordered_on_kernel(P relation,
...
bool is_ordered_on(R relation, vecmem::memory_resource& mr, vecmem::copy& copy,
    ...
    TRACCC_CUDA_ERROR_CHECK(cudaMemcpy(out.get(), &initial_out, sizeof(bool),
                                       cudaMemcpyHostToDevice));

    // Launch the kernel which will write its result to the `out` boolean.
    kernels::is_ordered_on_kernel<<<
        (n + block_size - 1) / block_size, block_size, 0,
        reinterpret_cast<cudaStream_t>(stream.cudaStream())>>>(relation, vector,
                                                               out.get());
    ...
    TRACCC_CUDA_ERROR_CHECK(
        cudaMemcpy(&host_out, out.get(), sizeof(bool), cudaMemcpyDeviceToHost));
