...
bool is_contiguous_on(P projection, vecmem::memory_resource& mr,
    ...
    TRACCC_CUDA_ERROR_CHECK(cudaMemcpy(iout_size.get(), &initial_iout_size,
                                       sizeof(uint32_t),
    ...
    TRACCC_CUDA_ERROR_CHECK(cudaMemcpy(&host_iout_size, iout_size.get(),
                                       sizeof(uint32_t),
                                       cudaMemcpyDeviceToHost));

    // Launch the second kernel, which will check if the values are unique.
    kernels::all_unique<<<(host_iout_size + block_size - 1) / block_size,
                          block_size>>>(iout.get(), host_iout_size, out.get());
