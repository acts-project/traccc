...
gbts_seeding_algorithm::output_type gbts_seeding_algorithm::operator()(
	...
	if(m_config.seed_ambi_params.sort_seeds) {
		...
		cudaMemcpy(ctx.h_seed_quality.get(), ctx.d_seed_quality, ctx.nSeeds*sizeof(float),  cudaMemcpyDeviceToHost);
		...
		auto seed_tmp = m_copy.get().to(output_seeds, *m_mr.host, nullptr, vecmem::copy::type::device_to_host);
		...
		m_copy.get()(vecmem::get_data(sorted_host_seeds), output_seeds, vecmem::copy::type::host_to_device)->wait();
	}
