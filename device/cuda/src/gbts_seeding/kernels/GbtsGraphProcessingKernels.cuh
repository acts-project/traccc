...
void __global__ gbts_seed_conversion_kernel(
    ...
    for (int prop_idx = threadIdx.x + blockIdx.x * blockDim.x;
        ...
        if ((best_for_hit < seed.size - 1) &
            (abs(curv_cot_1.y + curv_cot_2.y + curv_cot_3.y) < 3.0f*tight_bid_cot_threshold) &
            (seed.size < 5)) {
            continue;
        }
		...
		bool force_dropout = abs(curv_cot_1.x+curv_cot_2.x+curv_cot_3.x) < 3.0f*force_dropout_max_curv_m;
		force_dropout |= (abs(curv_cot_1.y+curv_cot_2.y+curv_cot_3.y) < 3.0f*tight_bid_cot_threshold) & diff_code == 0;
