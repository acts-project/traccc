...
#include "./kernels/fill_fitting_state_sort_keys.hpp"
...
/// Templated implementation of the CUDA track fitting algorithm starting from
/// track states.
///
...
track_state_container_types::buffer kalman_fitting_from_states(
    ...
    stream& str, unsigned int warp_size, bool states_are_fit) {
    ...
    const auto n_tracks = copy.get_size(track_states_view.headers);

    // Get the sizes of the track candidates in each track.
    const auto candidate_sizes = copy.get_sizes(track_states_view.items);
    ...
    fill_fitting_state_sort_keys(nBlocks, nThreads, stream, track_states_buffer,
...
///
template <typename detector_t, typename bfield_t>
track_state_container_types::buffer kalman_fitting_from_candidates(
    const typename detector_t::view_type& det_view, const bfield_t& field_view,
    ...
    return kalman_fitting_from_states<detector_t>(
        det_view, field_view, std::move(track_states_buffer), config, mr, copy,
        str, warp_size, false);
}
