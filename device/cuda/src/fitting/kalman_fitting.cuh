...
__global__ void fill_sequence_buffer(
    const edm::track_fit_container<default_algebra>::const_view track_fit_view,
    vecmem::data::jagged_vector_view<detray::geometry::barcode> barcode_view) {

    const typename edm::track_fit_collection<algebra_t>::const_device tracks(
        track_fit_view.tracks);
    const typename edm::track_state_collection<algebra_t>::const_device states(
        track_fit_view.states);
    const measurement_collection_types::const_device measurements(
        track_fit_view.measurements);
    vecmem::jagged_device_vector<detray::geometry::barcode> barcodes(
        barcode_view);
    ...
    for (const auto& bc : tracks.barcodes().at(tid)) {
        barcodes.at(tid).push_back(bc);
    }
}
...
kalman_fitting(
    ...
    if (!forward_on_first_iteration) {
        const unsigned int n_threads = 256;
        const unsigned int n_blocks =
            (static_cast<unsigned int>(n_tracks) + n_threads - 1) / n_threads;
        fill_sequence_buffer<default_algebra>
            <<<n_blocks, n_threads, 0, stream>>>(track_fit_view, seqs_buffer);
