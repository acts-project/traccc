...
__global__ void count_grid_capacities(
    ...
    vecmem::data::vector_view<const device::get_spacepoint_prefix_sum_element_t>
        sp_prefix_sum,
...
sp_grid_buffer spacepoint_binning::operator()(
    ...
    device::get_spacepoint_prefix_sum_result_t sp_prefix_sum =
        device::get_spacepoint_prefix_sum(spacepoints, m_mr);
    ...
    kernels::count_grid_capacities<<<num_blocks, num_threads>>>(
        m_config, m_axes.first, m_axes.second, traccc::get_data(spacepoints),
        vecmem::get_data(sp_prefix_sum), vecmem::get_data(grid_capacities));
    ...
    kernels::populate_grid<<<num_blocks, num_threads>>>(
        m_config, traccc::get_data(spacepoints),
        vecmem::get_data(sp_prefix_sum), grid_buffer);
    CUDA_ERROR_CHECK(cudaGetLastError());
    CUDA_ERROR_CHECK(cudaDeviceSynchronize());
