...
__global__ inline void gather_best_triplets_per_spacepoint(
    ...
    auto get_idx_for_state = [&triplets, &grid,
        ...
        if (state == 0) {
            const sp_location& spB_loc = this_triplet.spB;
            return grid.bin(spB_loc.bin_idx)[spB_loc.sp_idx];
        } else if (state == 1) {
            const sp_location& spM_loc = this_triplet.spM;
            return grid.bin(spM_loc.bin_idx)[spM_loc.sp_idx];
        } else {
            const sp_location& spT_loc = this_triplet.spT;
            return grid.bin(spT_loc.bin_idx)[spT_loc.sp_idx];
        }
    };
...
__global__ void select_seeds(
    ...
    device::select_seeds(details::global_index1(), filter_config, spacepoints,
                         ...
                         votes_per_triplet_view, seed_view);
}
