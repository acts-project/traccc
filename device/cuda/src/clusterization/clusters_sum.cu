...
__global__ void clusters_sum(
    ...
    auto prefix_sum = vecmem::device_atomic_ref<unsigned int>(total_clusters)
...
void clusters_sum(
    ...
    const unsigned int nDClustersSumBlocks =
        (n_modules + nClustersSumThreads - 1) / nClustersSumThreads;
