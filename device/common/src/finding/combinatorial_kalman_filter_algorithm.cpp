...
auto combinatorial_kalman_filter_algorithm::operator()(
    ...
    if (cfg.run_pkf && (cfg.max_num_branches_per_seed == 1 ||
        ...
        typename edm::track_container<algebra_t>::buffer
...

        {
            ...
            vecmem::data::vector_buffer<track_stats<scalar_t>>
                track_stats_buffer{n_seeds, mr().main};
            copy().setup(track_stats_buffer)->ignore();

            // Get the output track state data, depending on which data is
            // required
            vecmem::data::vector_buffer<track_state_candidate>
                track_cand_buffer{0, mr().main};
            vecmem::data::vector_buffer<
                filtered_track_state_candidate<algebra_t>>
            ...
            if (cfg.run_smoother == smoother_type::e_none) {
                ...
                filtered_track_cand_buffer = vecmem::data::vector_buffer<
                    filtered_track_state_candidate<algebra_t>>{max_cands,
                                                               mr().main};
            } else if (cfg.run_smoother == smoother_type::e_mbf) {
                full_track_cand_buffer = vecmem::data::vector_buffer<
                    full_track_state_candidate<algebra_t>>{max_cands,
                                                           mr().main};
            }
        ...
        /*if (cfg.run_smoother == smoother_type::e_kalman) {
            // Not needed
            vecmem::data::vector_buffer<unsigned int> param_ids_buffer(0u,
                                                                       mr().main);
            ...
            fit_backward<fitter_t>(nBlocks, nThreads, 0, stream,
                                   cfg.kalman_smoother,
        host_payload); TRACCC_CUDA_ERROR_CHECK(cudaGetLastError()); } else if
        (cfg.run_smoother == smoother_type::e_mbf) { std::cout
     ...
     *****************************************************************/
        ...
        for (unsigned int step = 0;
             step < cfg.max_track_candidates_per_track && n_in_params > 0;
             ++step) {
...

            {
                ...
                vecmem::data::vector_buffer<unsigned int>
                    out_params_index_buffer(n_in_params, mr().main);
        ...
        if (mr().host) {
            vecmem::async_size size =
                copy().get_size(tips_buffer, *(mr().host));
        ...
        if (n_tips_total > 0 && cfg.max_num_tracks_per_measurement > 0) {
            ...
            tip_to_output_map = vecmem::data::vector_buffer<unsigned int>(
                n_tips_total, mr().main);
        ...
        vecmem::copy::event_type ev =
            copy()(tip_length_buffer, tips_length_host);
        // Here we could give control back to the caller, once our code allows
        // for it. (coroutines...)
        ev->wait();
        ...
        if (n_tips_total > 0) {
            build_tracks_kernel(n_tips_total,
                                cfg.run_smoother == smoother_type::e_mbf,
