...
void select_seeds(
    ...
    for (unsigned int i = 0; i < num_triplets_per_bin; ++i) {
        ...
        if (n_triplets_per_spM >= filter_config.max_triplets_per_spM) {
            ...
            int min_index =
                min_elem(data, 0, filter_config.max_triplets_per_spM,
                         [&](triplet& lhs, triplet& rhs) {
                             return lhs.weight > rhs.weight;
                         });

            auto& min_weight = data[min_index].weight;
    ...
    insertionSort(data, 0, n_triplets_per_spM, [&](triplet& lhs, triplet& rhs) {
