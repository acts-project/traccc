...
TRACCC_HOST_DEVICE inline void build_tracks(
    ...
    using size_type = detray::dsize_type<default_algebra>;
    using bound_vector_type = traccc::bound_vector<default_algebra>;
    using bound_matrix_type = traccc::bound_matrix<default_algebra>;
    ...
    for (auto it = track.constituent_links().rbegin();
        ...
        if (run_mbf) {
            ...
            bound_track_parameters<default_algebra> smoothed_params;
            smoothed_params.vector() =
                predicted_vec - predicted_covariance * small_lambda_tilde;
            smoothed_params.covariance() =
                predicted_covariance -
                (predicted_covariance * big_lambda_tilde *
                 predicted_covariance);

            track_state.filtered_params() = filtered_params;
            track_state.smoothed_params() = smoothed_params;
