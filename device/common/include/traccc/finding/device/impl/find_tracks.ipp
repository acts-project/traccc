 ...
 */
TRACCC_DEVICE inline uint64_t encode_insertion_mutex(const bool locked,
                                                     const uint32_t size,
                                                     const float max) {
    ...
    const uint32_t lo = *reinterpret_cast<const uint32_t*>(&max);
...
TRACCC_HOST_DEVICE inline void find_tracks(
    ...
    while (barrier.blockOr(curr_meas < num_meas ||
        ...
        if (thread_id.getLocalThreadIdX() <
             ...
             * depth of if-statements which is important for code quality but,
             * more importantly, allows us to more easily use block-wide
             * synchronization primitives.
             */
            if (const traccc::scalar chi2 = trk_state.filtered_chi2();
                res == kalman_fitter_status::SUCCESS && chi2 < cfg.chi2_max) {
        ...
        while (barrier.blockOr(result.has_value())) {
            ...
            if (result.has_value()) {
                ...
                if (result.has_value() && !locked) {
                    new_size = size < cfg.max_num_branches_per_surface
                                   ? size + 1
                                   : size;
                    desired =
                        details::encode_insertion_mutex(true, new_size, max);
                     ...
                     */
                    unsigned long long int cas_grab_result =
                        atomicCAS(mutex_ptr, assumed, desired);

                    if (cas_grab_result == assumed) {
                        index = size;
                    }
                }
                ...
                if (index != std::numeric_limits<unsigned int>::max()) {
                     ...
                     */
                    unsigned long long int cas_release_result =
                        atomicCAS(mutex_ptr, desired,
                                  details::encode_insertion_mutex(
                                      false, new_size, new_max));

                    assert(cas_release_result = desired);
                }
            }
        }
    ...
    if (in_param_is_live) {
        if (local_num_params == 0) {
            ...
            out_params.at(out_offset - payload.curr_links_idx) =
                in_params.at(in_param_id);
            out_params_liveness.at(out_offset - payload.curr_links_idx) = 1u;
        } else {
            for (unsigned int i = 0; i < local_num_params; ++i) {
                ...
                out_params_liveness.at(out_offset - payload.curr_links_idx) =
                    1u;
