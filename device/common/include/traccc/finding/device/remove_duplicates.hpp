...
TRACCC_HOST_DEVICE inline void remove_duplicates(
    ...
    for (unsigned int i = min_tid; i <= max_tid; ++i) {
        ...
        const scalar chi2_per_dof_this =
            Lthis.chi2_sum / static_cast<scalar>(Lthis.ndf_sum - 5);
        const scalar chi2_per_dof_that =
         ...
         * $\chi^2$ score or, if the $\chi^2$ is equal, if the other track has
         * a lower identifier (this should be a very uncommon tie-breaker).
         */
