    ...
    TRACCC_HOST_DEVICE void operator()(
        ...
        if (cand.chi2 <= max_chi2) {
            ...
            if (cand.chi2 <= std::numeric_limits<scalar_t>::epsilon() &&
                ...
                if (!detray::algebra::approx_equal(
                        static_cast<float>(filtered_chi2), cand.chi2, 0.01f)) {
            ...
            switch (updater_state.m_run_smoother) {
                case smoother_type::e_mbf: {
                    ...
                    *track_cand_ptr = {cand.meas_idx, filtered_chi2,
                                       bound_param, predicted_params, full_jac};
                    break;
                }
