 ...
 public:
  ...
  class iterator {
   public:
    ...
    iterator(const global_neighborhood_indices& parent,
...
struct grid_helper_impl{
	...
	neighborhood_indices locNeighbors = std::get<N>(axes).get_neighborhood_indices(locIdx, sizes);
	neighborIndices.at(N) = locNeighbors;
	
	grid_helper_impl<N - 1>::get_neighborhood_indices(localIndices, sizes, axes,
							  neighborIndices);
    }
...
struct grid_helper {
    ...
    template <class... Axes>
    static global_neighborhood_indices<sizeof...(Axes)> get_neighborhood_indices(
      const std::array<size_t, sizeof...(Axes)>& localIndices,
      std::pair<size_t, size_t> sizes, const std::tuple<Axes...>& axes) {
	constexpr size_t MAX = sizeof...(Axes) - 1;
	...
	grid_helper_impl<MAX>::get_neighborhood_indices(localIndices, sizes, axes,
							neighborIndices);
    ...
    static global_neighborhood_indices<sizeof...(Axes)> get_neighborhood_indices(
      ...
      const std::tuple<Axes...>& axes) {
	return get_neighborhood_indices(localIndices, std::make_pair(size, size), axes);
  }
