...
void propagate_to_next_surface_debug::reset() {
    for (std::size_t i = 0;
         i <
         static_cast<std::size_t>(propagate_to_next_surface_failure_mode::MAX);
         ++i) {
        m_failure_count[i] = 0;
    }
}
...
void log_propagate_to_next_surface_debug(
    ...
    if (unsigned int n = obj.m_failure_count[static_cast<std::size_t>(
        ...
        n > 0) {
        TRACCC_DEBUG(n << " track candidates were killed due to detray "
                          "propagation failures");
    }

    if (unsigned int n = obj.m_failure_count[static_cast<std::size_t>(
            propagate_to_next_surface_failure_mode::HOLE_LIMIT_REACHED)];
        n > 0) {
        TRACCC_DEBUG(n << " tracks were killed due to reaching the hole limit");
    }

    if (unsigned int n = obj.m_failure_count[static_cast<std::size_t>(
            propagate_to_next_surface_failure_mode::BRANCH_LIMIT_REACHED)];
        n > 0) {
        TRACCC_DEBUG(
